...
They have therefore made it very difficult to scrape with all sorts of technical chicanery, 
...
embedded camera hardware using Edge-AI and Tiny-ML techniques. One advantage of doing the analysis locally at the camera is that 
the results of the image processing can be transmitted in real-time over very low bandwidth connections and over great distances, 
...
and the images themselves can be manually collected over WiFi during routine drive-by data collection or by daisy-chaining them in 
a WiFi grid network.
...
We ideally want about 100 images per brand for a healthy dataset for Machine Learning. 
